...
export class AuthenticationService {
  ...
  private _currentUser$ = new BehaviorSubject<boolean>(
    !!(this.currentUser && this.currentUser.token)
  );
  private _isAdmin$ = new BehaviorSubject<boolean>(
    this.currentUser ? this.currentUser.user.isAdmin : false
  );
  ...
  signUp(userData: userCredentials) {
    return this._http
      .post<IUserAuthentication>(`${this.rootUrl}/signUp`, userData)
      .pipe(
        tap((resp) =>
          resp.token
  ...
  login(username: string, password: string): Observable<IUserAuthentication> {
    return this._http
      .post<IUserAuthentication>(`${this.rootUrl}/signin`, {
        email: username,
        password,
      })
      .pipe(
